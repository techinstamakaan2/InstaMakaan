...
export const AspireCenturianParkCard = () => {
	...
	return (
		<div
			...
			className="group cursor-pointer w-full rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#15120c] shadow-md hover:shadow-xl hover:shadow-amber-500/10 hover:border-amber-400/40 transition-all duration-300 flex flex-col md:flex-row md:h-[320px]"
		>
			{/* ── Left: Image ── */}
			<div className="relative w-full md:w-[42%] h-52 md:h-full flex-shrink-0 overflow-hidden bg-slate-900">
				...
				<div className="absolute top-3 left-3">
					<span className="bg-amber-600/90 backdrop-blur-sm text-white text-[11px] font-semibold px-2.5 py-1 rounded-full shadow-sm">
						Luxury Residential
					</span>
				</div>
				...
				<div className="absolute top-3 right-3">
					<span className="flex items-center gap-1 bg-black/60 backdrop-blur-sm text-amber-300 text-[10px] font-bold px-2 py-1 rounded-full border border-amber-400/30">
			...
			<div className="flex-1 flex flex-col justify-between p-5 md:p-6 min-w-0">
				...
				<div>
					<div className="flex items-start justify-between gap-3 mb-2.5">
						...
						<span className="text-[11px] font-semibold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0">
							New Launch
						</span>
					</div>
					<div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-xs mb-1">
						<MapPin size={12} className="text-amber-600 dark:text-amber-500 flex-shrink-0" />
						<span className="truncate">Grand Luxury Residences · Techzone-4, Greater Noida (W)</span>
					</div>
					<div className="flex items-center gap-1.5 text-amber-700 dark:text-amber-400 text-xs mb-3">
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
						{STATS.map(({ label, value, accent }) => (
							<div key={label} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-2 text-center border border-slate-100 dark:border-slate-800">
								...
								<p className={`font-bold text-xs leading-tight break-words ${accent ? 'text-amber-600 dark:text-amber-400' : 'text-slate-800 dark:text-white'}`}>
									{value}
								</p>
							</div>
						))}
				...
				<div className="flex items-center justify-between gap-3 pt-3.5 mt-2 border-t border-slate-100 dark:border-slate-800">
					...
					<div className="flex items-center gap-2 ml-auto">
						<button
							...
							className="flex items-center gap-1.5 border border-amber-500/50 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 text-xs font-semibold px-3 py-2 rounded-lg transition-colors whitespace-nowrap"
						>
